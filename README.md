...
- [ ] **お問い合わせフォーム**は送信機能なし（`onsubmit="return false"`）。本番では送信先（フォームサービス or バックエンド）に接続。TOPのヒーローフォーム（FV先頭フォーム案 A2/B2/C2）にも「ご相談・備考」自由記入欄あり
- [ ] 許認可番号・料金・実績など、掲載に必要な正式情報の確認
- [ ] **対応エリアの関東地図（`img/kanto-map.png`）の利用ライセンス確認**。元画像は Adobe Stock のプレビュー（`360_F_420100459_...`）を白透過処理したもの。本公開前に正規ライセンス購入 or 自社素材へ差し替えること
